...
public class TestFilterer {
    public static boolean shouldRun(MixinVersions versions, Object testInstance) {
        if (testInstance instanceof RestrictedTest) {
            return ((RestrictedTest) testInstance).shouldRun(versions);
        }
    ...
    public static boolean shouldRun(MixinVersions versions, MixinTest ann) {
        ...
        if (versions.isFabric && !ann.minFabricMixin().isEmpty()) {
            if (Version.parse(ann.minFabricMixin()).isHigherThan(versions.mixinVersion)) {
                return false;
            }
        } else if (!ann.minMixin().isEmpty() && Version.parse(ann.minMixin()).isHigherThan(versions.upstreamMixinVersion())) {
            return false;
        }
        if (versions.hasMixinExtras() && !ann.minMixinExtras().isEmpty() && Version.parse(ann.minMixinExtras()).isHigherThan(versions.mixinExtrasVersion)) {
            return false;
        }
        return true;
    }
